...
phi, f, V, sigma, zeta, omega = symbols('phi f V sigma zeta omega', cls=IndexedBase)
x, y, z = symbols('x y z')
i, j, k = symbols('i j k', cls=Idx)
hx, hy, hz, r = symbols('hx hy hz r')
...
L1 = (1.0 / (r*r)) * ( (1.0 / (sympy.cos(phi[j])*sympy.cos(phi[j])))                                                          
    * D(sigma[k, j, i] * omega[k, j, i], x, x)
    + D(sigma[k, j, i] * omega[k, j, i], y, y)
    - sympy.tan(phi[j]) * D(sigma[k, j, i] * omega[k, j, i], y) )

L2 = f[j] * (zeta[k, j, i] + f[j]) * D(omega[k, j, i], z, z)

L3 = - f[j] * D(zeta[k, j, i], z, z) * omega[k, j, i]

L4 = (f[j] / r) * (D(V[k, j, i, 0], z, z) * D(omega[k, j, i], y)
      - D(V[k, j, i, 1], z, z) * (1.0 / sympy.cos(phi[j])) * D(omega[k, j, i], x)
      + D(V[k, j, i, 0], z) * D(omega[k, j, i], y, z)
      - D(V[k, j, i, 1], z) * (1.0 / sympy.cos(phi[j])) * D(omega[k, j, i], x, z))

L = (L1 + L2 + L3 + L4) * r*r* hx * hy * hz * sympy.cos(phi[j])
